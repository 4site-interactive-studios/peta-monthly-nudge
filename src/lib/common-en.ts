...
export abstract class CommonEN {
  constructor() {
    if (!CommonEN.enForm) {
  ...
  static setFieldValue(
    ...
    if (value === CommonEN.getFieldValue(name)) return;
    (document.getElementsByName(name) as NodeListOf<HTMLFormElement>).forEach(
      (field) => {
        if ("type" in field) {
          ...
          field.setAttribute("foursite-value-changed", "");
        }
      }
    );
  ...
  static createHiddenInput(name: string, value: string = "") {
    ...
    if (submitElement) {
      ...
      CommonEN.enForm.appendChild(formBlock);
    }
    return inputField;
  }
  ...
  static getDataCenter() {
    return CommonEN.getClientID() >= 10000 ? "us" : "ca";
  }
  ...
  static setBodyData(dataName: string, value: string | boolean) {
    ...
    if (typeof value === "boolean" && value === false) {
      body.removeAttribute(`data-foursite-${dataName}`);
      return;
    }
    body.setAttribute(`data-foursite-${dataName}`, value.toString());
  }

  // Get body data attributes
  static getBodyData(dataName: string) {
    const body = <HTMLBodyElement>document.querySelector("body");
    return body.getAttribute(`data-foursite-${dataName}`);
  }
  // Check if body has data attributes
  static hasBodyData(dataName: string) {
    const body = <HTMLBodyElement>document.querySelector("body");
    return body.hasAttribute(`data-foursite-${dataName}`);
  }
  ...
  static deepMerge(target: any, source: any) {
    for (const key in source) {
      if (source[key] instanceof Object)
        Object.assign(
          source[key],
          CommonEN.deepMerge(target[key], source[key])
        );
    }
  ...
  static getCurrencySymbol(): string {
    const currencyField = CommonEN.getField(
  ...
  static watchForError(callback: Function) {
    ...
    callbackType = "foursite" + capitalize(callbackType);
  ...
  static getPaymentType(): string {
    return CommonEN.getFieldValue("transaction.paymenttype");
  }
  // Set the Payment Type
  static setPaymentType(paymentType: string) {
    const enFieldPaymentType = CommonEN.getField(
